%%
%% latexmk -synctex=1 -interaction=nonstopmode -file-line-error -pdf keyshuffling.tex
%%
...
\author{Devon~``Plailect''~Maloney~{\relax(Contributor)}, 
        ``stuckpixel''~{\relax(Discoverer)}, 
        ``SciresM''~{\relax(Implementor)}, 
        ``Gelex''~{\relax(Contributor)}, 
        ``Normmatt''~{\relax(Contributor)}, 
        and~``Aurora~Wright''~{\relax(Implementor)}
        \vspace{-2.0em}}
...
decrypts the ARM9 firmware binary, which means we have 31 different keys for each
...
in memory which uses our crafted Key \#2 to deterministically decrypt the 10.2.0
